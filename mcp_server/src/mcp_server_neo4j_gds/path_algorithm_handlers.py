    ...
    def all_pairs_shortest_paths(self, db_url: str, username: str, password: str, **kwargs):
        ...
        with projected_graph(gds) as G:
            ...
            if apsp_data.empty:
                return {
                    "found": False,
                    "message": "No shortest paths found"
                }
    ...
    def random_walk(self, db_url: str, username: str, password: str, **kwargs):
        ...
        with projected_graph(gds) as G:
            ...
            if walk_data.empty:
                return {
                    "found": False,
                    "message": "No random walks generated"
                }
...
class LongestPathHandler(AlgorithmHandler):
    def longest_path(self, db_url: str, username: str, password: str, **kwargs):
        ...
        with projected_graph(gds) as G:
            ...
            result_data.sort(key=lambda x: x['longestPathCost'] if x['isFinite'] else float('inf'), reverse=True)
            ...
            return {
                ...
                "totalWeight": total_weight,
                "maxLongestPath": max([node['longestPathCost'] for node in result_data if node['isFinite']]) if any(node['isFinite'] for node in result_data) else 0,
